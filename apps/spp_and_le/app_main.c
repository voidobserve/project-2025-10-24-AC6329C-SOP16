...
void user_msg_handle_task(void)
{
    ...
    while (1)
    {
        ...
        case MSG_SEQUENCER_ONE_WIRE_SEND_INFO: // 使能单线发送
        {
            ...
            for (u8 i = 0; i < 5; i++) // 控制重复发送次数（只重复发5次、10次还是会有概率丢失数据；填20次后，暂时没有发现问题，就是电机反应会慢一些）
            // for (u8 i = 0; i < 20; i++) // 控制重复发送次数（只重复发5次、10次还是会有概率丢失数据；填20次后，暂时没有发现问题，就是电机反应会慢一些）
            {
