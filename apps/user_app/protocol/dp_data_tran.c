...
void parse_zd_data(unsigned char *LedCommand)
{
    ...
    if (LedCommand[0] == 0x01 && LedCommand[1] == 0x03) // 与APP同步数据
    {
        ...
        Send_buffer[8] = 0x01;
#if LED_STRIP_RGB
        Send_buffer[9] = 0x01; // 灯具类型：RGBC
#elif LED_STRIP_RGBW
        Send_buffer[9] = 0x02; // 灯具类型：RGBW
#elif LED_STRIP_RGBCW
        Send_buffer[9] = 0x03; // 灯具类型：RGBCW
#endif
        ...
        if (get_on_off_state())
        {
                ...
                LedCommand[2] >= 0 && LedCommand[2] <= 0x07)
            {
                                                              ...
                                                              // set_static_mode((WHITE >> 16) & 0xff, (WHITE >> 8) & 0xff, (WHITE >> 0) & 0xff);
                {
                    ...
                    fc_effect.rgb.b = 0;
                    fc_effect.Now_state = ACT_CW;
                    app_set_cw(100, 0x00); // 这个函数传参范围只能是0~100
                    set_fc_effect();
                }
                break; // WHITE

                case APP_COLORFUL_ANIMATION_INDEX_WARM_WHITE: // 静态暖白
                {
                    fc_effect.rgb.r = 0;
                    fc_effect.rgb.g = 0;
                    fc_effect.rgb.b = 0;
            ...
            //---------------------------------动态处理-----------------------------------
            if (LedCommand[0] == 0x04 && LedCommand[1] == 0x02 &&
                LedCommand[2] > 0x07 && LedCommand[2] <= APP_COLORFUL_ANIMATION_INDEX_WHITE_FLASH)
            {
            ...
            //---------------------------------CW（暖白光调节）控制----------------------------
            if (LedCommand[0] == 0x04 && LedCommand[1] == 0x07)
            {

                fc_effect.Now_state = ACT_CW;
                app_set_cw(LedCommand[3], LedCommand[2]);
            }
            //---------------------------------灯带长度-----------------------------------
